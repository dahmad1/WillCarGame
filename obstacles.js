var obsOrangeCar = new Image();
obsOrangeCar.src = "Images/spr_orange_car.png";
var obsMine = new Image();
obsMine.src = "Images/spr_mine.png";
...
function animateObstacles() {
  if (obsTimer > 0 && GAME.obstacles.length < GAME.level + 2) {
    obsTimer--;
  } else if (obsTimer == 0) {
    if (GAME.obsType == "car") {
      obsTimer = 100 - 10 * GAME.level + Math.floor(Math.random() * 150);
    } else if (GAME.obsType == "mine") {
      obsTimer = 50 - 2 * GAME.level + Math.floor(Math.random() * 75);
    }
